...
function DBTypeChanged() {
    if (($('#DBTypeId').val() == 2) && (!$('#oraclediv').is(":visible"))) {
        $('#oraclediv').toggle('show');
        $('#oraclediv').find('input, textarea', 'select').prop('disabled', false);
    }
    else if (($('#DBTypeId').val() != 2) && ($('#oraclediv').is(":visible"))) {
        $('#oraclediv').toggle('hide');
        $('#oraclediv').find('input, textarea', 'select').prop('disabled', true);
    }
}
